...
class ImportService
{
    ...
    public function importarPacientes(string $csvContent): array
    {
        ...
        foreach ($rows as $i => $row) {
            ...
            // Verificar duplicado
            if ($this->pacienteRepository->findOneBy(['rutHash' => $this->encryption->hmac($rut)]) !== null) {
                $omitidos++;
                continue;
            }
    ...
    public function importarTrabajadores(string $csvContent): array
    {
        ...
        foreach ($rows as $i => $row) {
            ...
            if ($this->trabajadorRepository->findOneBy(['rutHash' => $this->encryption->hmac($rut)]) !== null) {
                $omitidos++;
                continue;
            }
